...
function collectContractUrls(count) {
  ...
  for (const row of rows) {
    ...
    const args = Array.from(onclick.matchAll(/"([^"]*)"/g)).map(m => m[1]);
    if (args.length >= 4) {
      const u = new URL('/uiSNOC/main/service', origin);
      const p = new URLSearchParams();
      p.set('cat', 'viewTaskPage_cs');
      p.set('serviceName', args[0]);
      p.set('transactionId', args[1]);
      p.set('serviceTypeId', args[2]);
      p.set('action', (args[3] || 'edit'));
      u.search = p.toString();
      urls.push(u.href);
    }
  }
